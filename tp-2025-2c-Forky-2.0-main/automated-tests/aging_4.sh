...
# Script para ejecutar Query Control - AGING_4
...
echo -e "${BLUE}=========================================${NC}"
echo -e "${MAGENTA}       EJECUTANDO QUERY CONTROL        ${NC}"
echo -e "${MAGENTA}     AGING_4 - PRIORIDAD 1 (MÁXIMA)    ${NC}"
echo -e "${BLUE}=========================================${NC}"
echo -e "Archivo de configuración: ${YELLOW}$CONFIG_FILE${NC}"
echo -e "Archivo de queries: ${YELLOW}$QUERY_FILE${NC}"
echo -e "Prioridad: ${MAGENTA}$PRIORIDAD (MÁXIMA)${NC}"
...
if [ ! -f "$QUERY_FILE" ]; then
    ...
    echo -e "${YELLOW}💡 Archivos disponibles en el directorio:${NC}"
    ls -la AGING* 2>/dev/null || echo "  No hay archivos AGING*"
    exit 1
fi
...
echo -e "\n${GREEN}🚀 Iniciando Query Control - AGING_4 (MÁXIMA PRIORIDAD)...${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"

# Ejecutar Query Control con los parámetros
...
if [ $exit_code -eq 0 ]; then
    echo -e "${GREEN}✅ Query Control AGING_4 ejecutado correctamente${NC}"
else
    echo -e "${RED}❌ Query Control AGING_4 terminó con error (código: $exit_code)${NC}"
fi

echo -e "\n${BLUE}=========================================${NC}"
echo -e "${MAGENTA}     QUERY CONTROL AGING_4 FINALIZADO  ${NC}"
echo -e "${BLUE}=========================================${NC}"
