...
# Script para ejecutar Query Control
# Usa: query.config, query_instrucciones.txt y PRIORIDAD leída del config
...
QUERY_FILE="query_instrucciones.txt"

# Ruta completa al config
CONFIG_PATH="$QUERY_CONTROL_DIR/$CONFIG_FILE"

# Leer PRIORIDAD desde query.config
if [ -f "$CONFIG_PATH" ]; then
    PRIORIDAD=$(grep -E '^PRIORIDAD=' "$CONFIG_PATH" | head -n1 | cut -d'=' -f2)
    # Si no se encontró la línea o vino vacía, usar 1 por defecto
    if [ -z "$PRIORIDAD" ]; then
        PRIORIDAD="1"
    fi
else
    # Si no existe el config todavía, usar 1 por defecto
    PRIORIDAD="1"
fi
...
# Ejecutar Query Control con los parámetros calculados
