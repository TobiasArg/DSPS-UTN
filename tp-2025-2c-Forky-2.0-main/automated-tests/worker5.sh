...
# Script para ejecutar Worker 5
# Parámetros: Worker5.config, Worker ID 005
...
echo -e "${BLUE}=========================================${NC}"
echo -e "${BLUE}         EJECUTANDO WORKER 5            ${NC}"
echo -e "${BLUE}=========================================${NC}"
...
if [ $exit_code -eq 0 ]; then
    echo -e "${GREEN}✅ Worker 5 terminó correctamente${NC}"
else
    echo -e "${RED}❌ Worker 5 terminó con error ($exit_code)${NC}"
fi

echo -e "${BLUE}=========================================${NC}"
echo -e "${BLUE}      WORKER 5 FINALIZADO               ${NC}"
echo -e "${BLUE}=========================================${NC}"
exit $exit_code
